...
extends AMRMClientAsyncByz<T> {
  ...
  private static int NUM_BYZ_REPLICAS = 4;
  private int num_replicas = 0;


  public AMRMClientAsyncByzImpl(int intervalMs, CallbackHandler callbackHandler) {
    this(new AMRMClientImpl<T>(), intervalMs, callbackHandler);
  }
  
  @Private
  @VisibleForTesting
  public AMRMClientAsyncByzImpl(AMRMClient<T> client, int intervalMs,
