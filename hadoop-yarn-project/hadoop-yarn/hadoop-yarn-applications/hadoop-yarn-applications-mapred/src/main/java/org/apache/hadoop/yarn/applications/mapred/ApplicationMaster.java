...
package org.apache.hadoop.yarn.applications.mapred;
...
public class ApplicationMaster {
  ...
  public boolean init(String[] args) throws ParseException, IOException {
    ...
    opts.addOption("num_mappers", true, "No. of containers needed for mappers");
    opts.addOption("num_reducers", true, "No. of containers needed for reducers");
    opts.addOption("mapper_file", true, "Path to mapper script");
    opts.addOption("reducer_file", true, "Path to reducer script");
    opts.addOption("priority", true, "Application Priority. Default 0");
    opts.addOption("debug", false, "Dump out debug information");



    //I dont feel like changing the client too... just leave this here
    opts.addOption("num_containers", false, "This is an old option which is ignored");
    ...
    numMappers = Integer.parseInt(cliParser.getOptionValue(
        "num_mappers", "1"));

    numReducers = Integer.parseInt(cliParser.getOptionValue(
        "num_reducers", "1"));

    numTotalContainers = numMappers + numReducers; 


    mapper_file = cliParser.getOptionValue("mapper_file","");
    reducer_file = cliParser.getOptionValue("reducer_file","");

    if(numMappers <= 0){
        throw new IllegalArgumentException("Must have alteast one mapper");
    }

    if(numReducers <= 0){
        throw new IllegalArgumentException("Must have atleast one reducer");
    }


    if (numTotalContainers == 0) {
      throw new IllegalArgumentException(
          "Cannot run distributed shell mapred with no containers");
    }
  ...
  private class RMCallbackHandler implements AMRMClientAsync.CallbackHandler {
      ...
      for (ContainerStatus containerStatus : completedContainers) {
        ...
        if (0 != exitStatus) {
          ...
          if (numCompletedMappers.get() == numMappers){
            ...
            //clear reducer list so they do not get launched again
            allocatedReducers = new ArrayList<Container>();
          }

          //increment total number of completed containers
          numCompletedContainers.incrementAndGet();
    ...
    public void onContainersAllocated(List<Container> allocatedContainers) {
      ...
      for (Container allocatedContainer : allocatedContainers) {
        ...
        //first we need to launch the mappers.
        if(numLaunchedMappers.get() < numMappers){


            numLaunchedMappers.incrementAndGet();

            LOG.info("Launching mapper..." + numLaunchedMappers.get());
            LaunchContainerRunnable runnableLaunchContainer =
                new LaunchContainerRunnable(allocatedContainer, containerListener, 
                        Arrays.asList("python",  
                            System.getenv("HADOOP_PREFIX")+"/mapper.py",  
                            System.getenv("HADOOP_PREFIX")+"/map.txt", 
                            String.valueOf(numLaunchedMappers.get()), 
            ...
            LOG.info("Mappers have started storing reducer allocation");
            //put containers in list to save for reducers
            allocatedReducers.add(allocatedContainer); 
  ...
  private class LaunchContainerRunnable implements Runnable {
    ...
    public LaunchContainerRunnable(
        Container lcontainer, NMCallbackHandler containerListener, List<String> commands) {
